...
class WebsiteRAGSystem:
    ...
    async def process_user_image(self, uploaded_image) -> Dict:
        ...
        try:
            ...
            user_images_dir = os.path.join(self.crawled_images_dir, "user_uploads")
...
async def main():
    ...
    st.markdown(
        ...
        "Then, ask questions about the website content!"
    )
    
    # Initialize session state
    if 'rag_system' not in st.session_state:
        try:
            st.session_state.rag_system = WebsiteRAGSystem()
    ...
    if submit_button and website_url:
        try:
            ...
            st.error(f"❌ Error crawling website: {str(e)}")    # Query section
